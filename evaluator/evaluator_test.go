...
func testBooleanObject(t *testing.T, name string, obj object.Object, expected bool) bool {
	t.Helper()
	result, ok := obj.(*object.Boolean)
	if !ok {
		t.Errorf("'%s' object is not Boolean. got=%T (%+v)", name, obj, obj)
		return false
	}
	if result.Value != expected {
		t.Errorf("'%s' object has wrong value. got=%t, want=%t", name,
			result.Value, expected)
		return false
	}
	return true
...
`,
	...
	for i, tt := range tests {
		...
		if !ok {
			t.Errorf("[%d] no error object returned. got=%T(%+v)",
